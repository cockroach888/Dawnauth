    ...
    public class ModuleController : Controller
    {
        ...
        public ActionResult Search(FormCollection form)
        {
            ...
            if (sltModule > 0)
            {
                ...
                if (strWhere == null)
                {
                    strWhere = string.Format("charindex(',{0},',mdl_path)>0", sltModule);
                }
                else
                {
                    strWhere += string.Format(" and charindex(',{0},',mdl_path)>0", sltModule);
                }
            }
            //模块编码
            ...
            if (!string.IsNullOrEmpty(txtCode) && ValidHelper.EngIsEngAndNums(txtCode))
            {
                ...
                if (strWhere == null)
                {
                    strWhere = string.Format("charindex('{0}',mdl_code)>0", txtCode);
                }
                else
                {
                    strWhere += string.Format(" and charindex('{0}',mdl_code)>0", txtCode);
                }
            }
            ...
            int pageCount, recordCount;
            var dataList = DawnAuthModuleBLL.SelectPSPisAllPurposeRowNumber(dataPager.PageSize, dataPager.PageCurrent, "mdl_time desc,mdl_path", strWhere, out pageCount, out recordCount);
